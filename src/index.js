...
async function analyze(articles) {
  ...
  async function worker() {
    while (true) {
      ...
      await fetchArticlePage(targets[i]);
    }
  }
  ...
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/search") {
      ...
      try {
        ...
        if (!articles.length) return json({ keyword, count: 0, articles: [] });

        articles = await analyze(articles);
        return json({
          keyword,
          count: articles.length,
        ...
        return json({ error: `검색/분석 중 오류: ${e.message || e.name || "알 수 없는 오류"}` }, 502);
      }
    }
